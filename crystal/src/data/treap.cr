...
class Treap(T)
  ...
  def erase(x : T);@root = erase @root, x;end
  def erase_all(x : T)
    a, m = split @root, x
    _, b = splitl m, x
    @root = merge a, b
  ...
  def count(x : T) : Int32;index_rg(@root, x) - index @root, x;end
  def find(x : T) : Bool
    t = @root
    return false if !t
    while t
  ...
  private def insert(t : Node(T)?, node : Node(T)) : Node(T)?
    return node if !t
    if node.pri > t.try &.pri
      node.l, node.r = split t, node.key
      update node
    elsif node.key < t.try &.key
      nnl(t).l = insert t.try(&.l), node
      update t
    else
      nnl(t).r = insert t.try(&.r), node
      update t
    end
  end
  private def erase(t : Node(T)?, x : T) : Node(T)?
    return nil if !t
    if x == t.try &.key
      merge t.try(&.l), t.try &.r
    elsif x < t.try &.key
      nnl(t).l = erase t.try(&.l), x
      update t
    else
      nnl(t).r = erase t.try(&.r), x
      update t
    end
  end
  # コンテナが空でないことを前提とする
