class Sortedsumlist(T)
	struct Iterator(T)
		def initialize(@outer : Int32, @inner : Int32, @bag : Array(Array(T)), @sums : Array(T));end
		def pos;{@outer,@inner};end
		def v;@bag[@outer][@inner];end
		def v=(x : T);@sums[@outer]-=v()-x;@bag[@outer][@inner]=x;end
		def +(n : Int32)
			...
			Iterator(T).new(outer, inner, @bag, @sums)
		end
	...
	def head;Iterator(T).new(0, 0, @list, @blk);end
	def tail;Iterator(T).new(@list.size, 0, @list, @blk);end
	RATIO = 8
	BOUND = 20
	def initialize;@size = 0;@list = [] of Array(T);@blk = [] of T;end
	def initialize(@size : Int32, @list : Array(Array(T)), @blk : Array(T));end
	def initialize(p : Array(T))
		...
		@blk = Array(T).new(s,0)
		z.sort!
		s.times { |i| @list[i] = z[(@size*i// s)...(@size*(i + 1)// s)];@blk[i] = @list[i].sum }
	end
	def empty?;@size == 0;end
	def size;@size;end
	def clear;@size = 0; @list.clear;end
	def clone;Sortedlist(T).new(@size,@list,@blk);end
	def _position(x : T)
		sz = @list.size
	...
	def _pop(bi, i)
		...
		@blk[bi] -= v
		@size -= 1
		(@list.delete_at(bi);@blk.delete_at(bi)) if @list[bi].empty?
		v
	end
	def <<(x : T);insert(x);end
	def insert(x : T)
		(@list = [[x]];@blk=[x]; @size = 1; return) if empty?
		bi, i = _position(x)
		@list[bi].insert(i, x)
		@blk[bi]+=x
		@size += 1
	...
	def []=(i : Int32, x : T)
		if i >= 0
			@list.size.times do |b|
				(@blk[b]-=@list[b][i]-x;return @list[b][i] = x) if i < @list[b].size
				i -= @list[b].size
			end
		else
			(@list.size - 1).downto(0) do |b|
				i += @list[b].size
				(@blk[b]-=@list[b][i]-x;return @list[b][i] = x) if i >= 0
			end
		end
		@list[-1][-1]
	end
	...
	def lower_bound(x : T)
		@list.size.times { |i| return Iterator(T).new(i, @list[i].bsearch_index { |v| v >= x }.not_nil!,@list, @blk) if !@list[i].empty? && @list[i][-1] >= x }
		tail
	end
	def upper_bound(x : T)
		@list.size.times { |i| return Iterator(T).new(i, @list[i].bsearch_index { |v| v > x }.not_nil!,@list, @blk) if !@list[i].empty? && @list[i][-1] > x }
	...
	def sum(l : Int32, r : Int32)
		return 0 if l >= r || empty?
		i, bi = l, 0
		while i >= @list[bi].size
			i -= @list[bi].size
			bi += 1
		end
		j, bj = r, 0
		while j > @list[bj].size
			j -= @list[bj].size
			bj += 1
		end
		return @list[bi][i...j].sum if bi == bj
		@list[bi][i..].sum + @list[bj][...j].sum + @list[(bi+1)...bj].sum{|v|v.sum}
	end
	def sum(l : Iterator(T), r : Iterator(T))
		return 0 if l >= r || empty?
		bi,i=l.pos
		bj,j=r.pos
		return @list[bi][i...j].sum if bi == bj
